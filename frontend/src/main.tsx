...
function VoiceControl() {
  const [settings, setSettings] = useState<any>({ name: 'Local voice agent', base_url: '', health_path: '/health', call_path: '/calls', enabled: false, config: {} });
  const [message, setMessage] = useState('');
  useEffect(() => { void api('/admin/voice-agent').then(setSettings).catch(error => setMessage(error.message)); }, []);
  const updateConfig = (key: string, value: string) => setSettings((current: any) => ({ ...current, config: { ...(current.config || {}), [key]: value } }));
  const save = async () => { try { const result = await api('/admin/voice-agent', { method: 'PUT', body: JSON.stringify(settings) }); setSettings(result); setMessage('Voice AI instructions saved'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Voice settings save failed'); } };
  return <section><p className="eyebrow">VOICE RUNTIME</p><h1>Voice AI Control</h1><Notice message={message}/><div className="panel"><p className="muted">Live calling remains unavailable until a real calling provider is configured and health-verified.</p><label>System instructions<textarea value={settings.config?.system_instructions || ''} onChange={event => updateConfig('system_instructions', event.target.value)}/></label><label>Voice personality<textarea value={settings.config?.voice_personality || ''} onChange={event => updateConfig('voice_personality', event.target.value)}/></label><label>Opening style<textarea value={settings.config?.opening_style || ''} onChange={event => updateConfig('opening_style', event.target.value)}/></label><label>Objection handling<textarea value={settings.config?.objection_handling || ''} onChange={event => updateConfig('objection_handling', event.target.value)}/></label><label>Portfolio response<textarea value={settings.config?.portfolio || ''} onChange={event => updateConfig('portfolio', event.target.value)}/></label><label>Pricing response<textarea value={settings.config?.pricing_rules || ''} onChange={event => updateConfig('pricing_rules', event.target.value)}/></label><button onClick={() => void save()}>Save Voice Instructions</button></div></section>;
}
      ...
      <div className="shell-frame">
        ...
        <main>
          <Routes>
            ...
            <Route path="/admin/voice-agent" element={<VoiceControl/>}/>
            <Route path="/voice-agent" element={<VoiceControl/>}/>
