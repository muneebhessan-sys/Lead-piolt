...
class FacebookProvider(MessageProvider):
    name = "FACEBOOK"

    def __init__(self, page_access_token: str = "", api_version: str = "v20.0", base_url: str = "https://graph.facebook.com") -> None:
    ...
    async def send(self, to: str, body: str, context: dict[str, Any] | None = None) -> SendResult:
        ...
        if not to.strip() or not body.strip():
            raise ValueError("FACEBOOK_RECIPIENT_AND_BODY_REQUIRED")
        url = f"{self.base_url}/{self.api_version}/me/messages"
        payload = {"recipient": {"id": to}, "message": {"text": body}}
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(url, json=payload, params={"access_token": self.page_access_token})
        if response.is_error:
            raise ValueError(f"FACEBOOK_SEND_FAILED: HTTP_{response.status_code}")
        data = response.json()
        provider_id = data.get("message_id") or data.get("id")
        if not provider_id:
            raise ValueError("FACEBOOK_SEND_FAILED: PROVIDER_ID_MISSING")
        return SendResult({
            "status": "SENT",
            "channel": self.name,
            "recipient": to,
